...
public class AddStreamTs {
    ...
    private static final String TARGET_DIR = "/app/mistserver_64_3.5.1/media"; // 타겟 디렉토리 (리눅스)
    ...
    private static void processNewFile(String sourcePath, String filename) {
        try {
            // 1. MP4 → TS 변환
            String tsFilePath = convertToTs(sourcePath);
            String tsFilename = filename.replace(".mp4", ".ts");
            
            // 2. TS 파일 업로드
            String remoteFilePath = uploadFileToServer(tsFilePath, tsFilename);
            
            // 3. MistServer 스트림 추가
            addStreamToMistServer(remoteFilePath, tsFilename);
        } catch (Exception e) {
            System.err.println("Failed to process file: " + e.getMessage());
        }
    }

    // FFmpeg를 이용한 TS 변환 메소드
    private static String convertToTs(String mp4Path) throws IOException, InterruptedException {
        String tsPath = mp4Path.replace(".mp4", ".ts");
        
        ProcessBuilder processBuilder = new ProcessBuilder(
            "ffmpeg",
            "-i", mp4Path,
            "-c:v", "copy",  // 비디오 코덱 변경 없음
            "-c:a", "copy",  // 오디오 코덱 변경 없음
            "-f", "mpegts",  // 출력 형식 설정
            tsPath
        );
