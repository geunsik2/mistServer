...
### Trouble Shooting
- 스트림 등록은 되지만 재생이 안 되는 경우가 있다.
    - mp4 파일을 ts 파일로 변환 후 등록하면 재생된다.
        - 변환은 ffmpeg 코덱을 사용한다.
        - 스트림명을 파일명과 다르게 등록해야 한다.
            - ex) 파일명이 <123.ts>인 경우 <123ts.ts>으로 등록한다.
|특징|MP4|MPEG-TS|
| :-:  | :-: | :-: |
|목적|저장 및 재생용(온디맨드 콘텐츠에 적합)|실시간 스트리밍에 최적화|
